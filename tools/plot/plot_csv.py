...
for configline in configlines:
	if '=' in configline:
		...
		if data[0] == 'title':
			title = data[1]
		elif data[0] == 'speedup' and data[1].startswith('1'):
			speedup=True
	else: 
...
for parts in partitioners:
	...
	for tcs in allTC:
		...
		reader = csv.reader(f)
		header = reader.next()
		if not header[len(header)-1] == 'lastSteadyMult':
			print tcolor.FAIL + 'Warning: mailformed csv (' + file + ')' + tcolor.ENDC
			sys.exit(-1)
		outputItems = reader.next()[len(header)-1]
